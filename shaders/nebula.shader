...
uniform float star_density : hint_range(1,200)=16;
uniform float star_brightness : hint_range(0,1)=0.5;
uniform int nebula_seed : hint_range(4,50)=34;
...
vec3 permute(vec3 x) 
{
	float nseed = float(nebula_seed);
	return mod289(((x*nseed)+1.0)*x);
}
...
float stars(in vec2 x, float numCells, float size, float br)
{
	...
	return br * (smoothstep(.97, 1, (1. - sqrt(d))));
}
...
vec4 mainImage(vec2 fragCoord,float time)
{
	...
	nebulaColor1 = hsv2rgb(vec3(0.5+0.5*sin(time*0.01), 0.5, 0.25));
	nebulaColor2 = hsv2rgb(vec3(0.5+0.5*sin(time*0.021), 1.0, 0.25));

	result += fractalNebula(coord + vec2(.1, .1), nebulaColor1, 1.);
	result += fractalNebula(coord + vec2(0.7, .2), nebulaColor2, .5);
	result += stars(coord, star_density, .025, star_brightness) * vec3(.9, .9, .95);
