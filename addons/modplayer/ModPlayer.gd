...
class GodotModPlayerChannelStatus:
	...
	func get_volume_db( ) -> float:
		...
		return self.volume_table[int(v * 64)]
		...
		for sample in t.samples:
			...
			if not( id in loaded ):
				...
				if sample.loop_type & Mod.ModLoopType.FORWARD_LOOP != 0:
					...
					printt( "PINGPONG", ass.loop_begin, ass.loop_end )
				loaded[id] = ass
			else:
				ass = loaded[id]
...
func set_volume_db( vdb:float ):
	...
	if master_bus_id == -1:
		return
	AudioServer.set_bus_volume_db( master_bus_id, self.volume_db )
...
func set_tempo( _tempo:float ):
	tempo = _tempo
	self.tick_per_second = 1.0 / ( tempo * ( chip_speed / 125.0 ) )
...
func _process_update_channels( ) -> void:
	if self.channel_audio_effects.size() == 0:
		self.stop()
		return
	for channel in self.channel_status:
		channel.update( )
